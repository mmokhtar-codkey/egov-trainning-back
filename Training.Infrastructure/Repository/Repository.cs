...
using Training.Common.Infrastructure.Repository;
...
namespace Training.Infrastructure.Repository
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        public async Task<(int Count, IEnumerable<T> Result)> FindPagedAsync(Expression<Func<T, bool>> predicate = null, int pageNumber = 0, int pageSize = 0, IEnumerable<SortModel> orderByCriteria = null, Func<IQueryable<T>, IIncludableQueryable<T, object>> include = null, bool disableTracking = true)
        {
            IQueryable<T> query = DbSet;
            if (disableTracking)
            {
        ...
        public async Task<TType> GetSelectSingleAsync<TType>(Expression<Func<T, bool>> where, Expression<Func<T, TType>> select) where TType : class
        {
            IQueryable<T> query = DbSet;
            return await query.Where(where).Select(select).FirstOrDefaultAsync();
        }
